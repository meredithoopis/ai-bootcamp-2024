...
class SemanticVectorStore(BaseVectorStore):
    ...
    def _calculate_similarity(
        ...
        # HINT: np.dot
        "Your code here"
        dproduct_arr = [np.dot(dembed_np, per_embed_np) for per_embed_np in dembed_np]
        # calculate the cosine similarity
        # by dividing the dot product by the norm
        # HINT: np.linalg.norm
        "Your code here"
        cos_sim_arr = np.array(dproduct_arr) / (np.linalg.norm(dembed_np, axis = 1) * np.linalg.norm(qembed_np))
